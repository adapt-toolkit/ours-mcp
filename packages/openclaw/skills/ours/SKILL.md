...
5. **(Optional) Wake on mail.** Wake is enabled **in-session by you**, after an identity is
   ...
   via OpenClaw's shell tool and react to each new message from that loop (see *Getting woken on new mail*
...
2. **Wake check.** The `choose_identity` / `create_identity` response may prompt you to "arm a
   ...
   loop — see *Getting woken on new mail*). If the user wants live reactivity for the just-bound identity,
