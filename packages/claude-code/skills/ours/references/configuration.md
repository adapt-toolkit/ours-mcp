...
**How each caller picks a daemon today:**

| Caller | Selector |
|---|---|
| `ours-mcp <cmd>` (start/stop/status/watch/create-root) | env vars in *its own* shell, else `OURS_CONFIG`/`~/.ours/config.json` |
| The Claude Code connector | **inherits the environment of the Claude process** — there is no per-workspace selector yet |
| Any MCP client spawning `ours-mcp proxy` | same: whatever environment it was launched with |

That middle row is the real limitation: to point a Claude Code session at a
...
first-class per-workspace selector) are planned; they do not exist yet.
