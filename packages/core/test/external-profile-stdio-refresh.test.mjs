...
try {
  ...
  let buffer = ''; const pending = new Map(); let stderr = ''; proxy.stderr.on('data', (value) => { stderr += value; });
  proxy.stdout.on('data', (value) => { buffer += value; let index; while ((index = buffer.indexOf('\n')) >= 0) { const line = buffer.slice(0, index); buffer = buffer.slice(index + 1); try { const frame = JSON.parse(line); if (frame.id !== undefined && pending.has(frame.id)) { pending.get(frame.id)(frame); pending.delete(frame.id); } } catch {} } });
  ...
  assert.equal((await fetch(`${endpoint}/version`, { headers: { authorization: `Bearer ${oldToken}` } })).status, 401, 'old token is rejected after official update');
  ...
  proxy.stdin.end(); const [proxyCode] = await once(proxy, 'exit'); assert.equal(proxyCode, 0, stderr);
  const retained = await attachOursClient({ endpoint, expectedInstanceId, credentialPath, sessionMode: 'external', leaseToken: 'fixture-owner-opaque', env: {} });
...
} finally { if (proxy && proxy.exitCode === null) { try { proxy.stdin.end(); } catch {} await Promise.race([once(proxy, 'exit'), pause(1000)]); if (proxy.exitCode === null) proxy.kill('SIGKILL'); } if (daemon.exitCode === null) daemon.kill('SIGTERM'); await Promise.race([once(daemon, 'exit'), pause(7000)]); if (daemon.exitCode === null) daemon.kill('SIGKILL'); }
