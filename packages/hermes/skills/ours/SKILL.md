...
- Identities and their state (contacts, inbox, keys) persist under the daemon's state dir
  ...
  Hermes agents and sessions on this host. A host can run a second one, but only with **both**
  a different port and a different state dir — two daemons may never share a state dir, and
  each is a separate presence with its own identities (see `references/configuration.md`).
