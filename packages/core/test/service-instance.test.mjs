...
const dedicated = buildSystemdUnit({
  instance: 'tg', port: 3060, brokerUrl: 'wss://broker1.ours.network', stateDir: '/home/u/.ours-tg',
  execPath: '/usr/bin/node', self: '/opt/ours/cli.js',
});
...
const plistDedicated = buildLaunchdPlist({
  instance: 'rooms', port: 3062, brokerUrl: 'wss://b', stateDir: '/s2', execPath: '/n', self: '/c', logPath: '/l',
});
...
console.log('  ✓ unit/plist bake the instance and its own port + state dir');

// ---- determinism / idempotency: same input ⇒ byte-identical definition ------
assert.equal(
  buildSystemdUnit({ instance: 'tg', port: 3060, brokerUrl: 'wss://b', stateDir: '/s', execPath: '/n', self: '/c' }),
  buildSystemdUnit({ instance: 'tg', port: 3060, brokerUrl: 'wss://b', stateDir: '/s', execPath: '/n', self: '/c' }),
