...
cp "$core_dir/tests/notif_actor.mu" "$core_dir/tests/protocol_container.mm" "$peer_build/"
cp "$repo_root/packages/core/mufl_code/config.mufl" "$peer_build/config.mufl"
sed -i '0,/loads libraries/s//loads libraries\n    registration_proof,/' "$peer_build/notif_actor.mu"

echo "compiling notification peer/service fixture from source …"
(
  cd "$peer_build"
  MUFL_STDLIB_PATH="$toolkit/mufl_stdlib" \
    "$compiler" -mp "$toolkit/meta" -mp "$toolkit/transactions" notif_actor.mu >/dev/null
)

port="${NOTIFICATIONS_TEST_PORT:-19878}"
...
MCP_UNIT_DIR="$repo_root/packages/core/mufl_code" \
PEER_UNIT_DIR="$peer_build" \
  node "$repo_root/packages/core/test/notifications-mufl.test.mjs"
