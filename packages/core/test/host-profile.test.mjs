...
assert.deepEqual(readHostProfile(profile), { ...tuple, endpoint: 'http://127.0.0.1:8787' }, 'complete tuple is selected and normalized');

const empty = join(root, 'empty.json');
writeFileSync(empty, '{}\n', { mode: 0o600 });
assert.equal(readHostProfile(empty), null, 'ordinary config does not select profile mode');

const partial = join(root, 'partial.json');
writeFileSync(partial, JSON.stringify({ endpoint: tuple.endpoint }), { mode: 0o600 });
assert.throws(() => readHostProfile(partial), /complete host profile/i, 'partial file tuple cannot fall through');
assert.throws(() => validateHostProfile({ endpoint: tuple.endpoint }), /complete host profile/i, 'partial programmatic tuple is rejected');

const mixed = join(root, 'mixed.json');
writeFileSync(mixed, JSON.stringify({ ...tuple, port: 1234 }), { mode: 0o600 });
assert.throws(() => readHostProfile(mixed), /legacy selection/i, 'legacy selectors cannot mix with profile');

chmodSync(profile, 0o644);
assert.throws(() => readHostProfile(profile), /private|permission/i, 'group-readable profile is rejected');
chmodSync(profile, 0o600);

assert.deepEqual(hostProfileFromEnv({ OURS_CONFIG: profile }), { ...tuple, endpoint: 'http://127.0.0.1:8787' }, 'explicit config selects profile');
assert.throws(() => hostProfileFromEnv({ OURS_CONFIG: join(root, 'absent.json') }), /cannot read/i, 'unreadable explicit config refuses');
assert.throws(() => hostProfileFromEnv({ OURS_CONFIG: profile, OURS_API_TOKEN: 'legacy' }), /conflicts/i, 'legacy credentials conflict with profile');
...
const defaultConfigDir = join(defaultHome, '.ours');
mkdirSync(defaultConfigDir, { recursive: true, mode: 0o700 });
const defaultProfile = join(defaultConfigDir, 'config.json');
...
assert.match(partialSelection.stderr, /complete host profile/i, 'partial programmatic external selection refuses before legacy attachment');
...
const managedTuple = { ...tuple, endpoint: 'http://127.0.0.1:9876' };
...
assert.deepEqual(hostProfileFromEnv({ HOME: defaultHome, OURS_CONFIG: profile }), { ...tuple, endpoint: 'http://127.0.0.1:8787' });
for (const invalid of ['{}', '{broken', JSON.stringify({ composeFile: '/not-a-fallback' })]) {
  writeFileSync(managedPath, invalid);
  assert.throws(() => hostProfileFromEnv({ HOME: defaultHome }), /host profile/i);
}
